...
from status import status_menu_portal  # Import the status management engine
    ...
    while True:
        try:
            ...
            if command == "-help":
                ...
                print("[System Debug] Sending credentials to authentication module...")
                
                try:
                    result = validate_login(username, password)
                    print(f"[System Debug] Module raw return response: {result}")
                    ...
                    if success:
                        ...
                        print(f"Rights  : {str(user_role).upper() if user_role else 'NONE'}")
                        print("="*40 + "\n")
                    else:
                        print("\n[Access Denied] Invalid username or password. Please try again.\n")
                        
                except Exception as inner_error:
                    print(f"\n[Critical Execution Error] Validation engine failed:")
                ...
                # Handle hitting the enter key with an empty space gracefully
                continue
            else:
            ...
            print("\n" + "!"*50)
            print("CRITICAL SHELL INTERFACE LOOP EXCEPTION OCCURRED:")
            print(traceback.format_exc())
            print("!"*50 + "\n")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[Fatal Initialization Failure] System main runtime could not compile:")
        print(traceback.format_exc())
